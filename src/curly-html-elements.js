export default class CurlyElements {
    ...
    render() {
        ...
        childItems.forEach((child, index) => {
            if (coordinates[index]) {
                ...
                const left = coordinates[index][0] - (child.clientWidth / 2) + this._options.shiftX;
                const top = coordinates[index][1] - (child.clientHeight / 2) + this._options.shiftY;
    ...
    _computeDpt(r1, r2, theta, rotateY = 0) {
    ...
    equalCoordinates(n, r1, r2, centerX, centerY) {
        ...
        for (let i = 1; i < numIntegrals; i++) {
            theta += i * deltaTheta;
            dpt = this._computeDpt(r1, r2, theta);
            circ += dpt;
        }
        ...
        for (let i = 0; i < numIntegrals; i++) {
            ...
            run += this._computeDpt(r1, r2, theta, rotateY);
        }
